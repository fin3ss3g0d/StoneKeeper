...
; This function consumes exactly 336 bytes of stack space
; sub rsp, 28h used four times and totals 40 bytes each time (40 * 4 = 160)
; There are 22 push instructions, each of which consumes 8 bytes (8 * 22 = 176)
; 160 + 176 = 336
QuadSleep PROC
    ...
    push r9                                            ; Push SleepEx onto the stack

    mov r10, 0FFFFFFFFh                                ; Set up the first argument to SleepEx as INFINITE
    push r10                                           ; Push INFINITE onto the stack
    push rcx                                           ; Push rcx gadget onto the stack (pop rcx; ret)
    mov r10, 1                                         ; Set up the second argument to SleepEx as TRUE
    push r10                                           ; Push TRUE onto the stack
    push rdx                                           ; Push rdx gadget onto the stack (pop rdx; ret)

    lea r10, [rdx + 1]                                 ; pop rdx; ret + 1 = ret (SleepEx return - "ret" placed into r10)
    push r10                                           ; push r10 (ret) onto stack

    sub rsp, 28h                                       ; Adjust the stack pointer to create 40 bytes of space

    push r8                                            ; Push shadow fixer gadget onto the stack (add rsp, 20h; pop rdi; ret)
    push r9                                            ; Push SleepEx onto the stack
    mov r10, 0FFFFFFFFh                                ; Set up the first argument to SleepEx as INFINITE
    push r10                                           ; Push INFINITE onto the stack
    push rcx                                           ; Push rcx gadget onto the stack (pop rcx; ret)
    ...
    push r8                                            ; Push shadow fixer gadget onto the stack (add rsp, 20h; pop rdi; ret)
    mov rcx, 0FFFFFFFFh                                ; Set up the first argument to SleepEx as INFINITE
    mov rdx, 1                                         ; Set up the second argument to SleepEx as TRUE

    jmp r9                                             ; Jump to SleepEx
...
PentaWaitExAndDelay PROC
    ...
    push rbx										   ; Save the original value of rbx (WaitEx address) (arg5)

    mov r10, OFFSET end_label                          ; Place the address of end_label into r10
    push r10                                           ; Push r10 (end_label) onto the stack

    mov rbx, [rsp + 68h]                               ; Set up WaitForSingleObjectEx address in rbx (arg5 - PVOID) (40x1 + 8x3 + 40 (28h))
    
    mov r10, [rsp + 70h]                               ; Set up NtDelayExecution address in r10 (arg6 - PVOID) (40x1 + 8x3 + 48 (30h))
    ...
    mov r10, [rsp + 90h]
    push r10                                           ; Push arg7 (PLARGE_INTEGER - INFINITE) onto the stack
    push rdx										   ; Push rdx gadget onto the stack (pop rdx; ret)
    ...
    push rbx										   ; Push WaitForSingleObjectEx address onto the stack
    mov r10, [rsp + 100h]                              ; Set up the first argument as the fourth timer object (arg11 - HANDLE) (40x2 + 8x11 + 88 (58h))
    push r10                                           ; Push timer handle onto the stack
    push rcx										   ; Push rcx gadget onto the stack (pop rcx; ret)

    mov r10, 0FFFFFFFFh							       ; Set up the second argument as INFINITE
    push r10                                           ; Push INFINITE onto the stack
    push rdx										   ; Push rdx gadget onto the stack (pop rdx; ret)

    mov r10, 1										   ; Set up the third argument as TRUE
    push r10                                           ; Push TRUE onto the stack
    push r9										   	   ; Push r8 gadget onto the stack (pop r8; ret)

    lea r10, [r9 + 2]                                  ; pop r8; ret + 2 = ret (WaitForSingleObjectEx return - "ret" placed into r10)
    push r10                                           ; push r10 (ret) onto stack
    ...
    push r12                                           ; Push shadow fixer gadget onto the stack (add rsp, 20h; pop rdi; ret)
    push rbx										   ; Push WaitForSingleObjectEx address onto the stack
    mov r10, [rsp + 168h]                              ; Set up the first argument as the third timer object (arg10 - HANDLE) (40x3 + 8x20 + 80 (50h))
    push r10                                           ; Push timer handle onto the stack
    push rcx										   ; Push rcx gadget onto the stack (pop rcx; ret)
    ...
    push r12                                           ; Push shadow fixer gadget onto the stack (add rsp, 20h; pop rdi; ret)
    push rbx										   ; Push WaitForSingleObjectEx address onto the stack
    mov r10, [rsp + 1D0h]                              ; Set up the first argument as the second timer object (arg9 - HANDLE) (40x4 + 8x29 + 72 (48h))
    push r10                                           ; Push timer handle onto the stack
    push rcx										   ; Push rcx gadget onto the stack (pop rcx; ret)
    ...
    push r12                                           ; Push shadow fixer gadget onto the stack (add rsp, 20h; pop rdi; ret)
    mov rcx, [rsp + 230h]                              ; Set up the first argument as the first timer object (arg8 - HANDLE)
    mov rdx, 0FFFFFFFFh							       ; Set up the second argument as INFINITE
    mov r8, 1										   ; Set up the third argument as TRUE

    jmp rbx                                            ; Jump to WaitForSingleObjectEx
...
PentaWaitExAndDelay ENDP

PentaNtWaitAndDelay PROC
    ; Assuming rsp points to the return address upon function entry (original calculation)
    ; Accessing arguments five through ten after the initial stack setup (testing)
    ;mov rax, [rsp + 28h] ; Access arg5 (fifth argument)
    ;mov rbx, [rsp + 30h] ; Access arg6 (sixth argument)
    ...
    mov r10, [rsp + 108h]                              ; Set up the first argument as the fourth timer object (arg11 - HANDLE) (40x2 + 8x12 + 88 (58h))
    push r10                                           ; Push timer handle onto the stack
    push rcx                                           ; Push rcx gadget onto the stack (pop rcx; ret)
    ...
    push r12                                           ; Push shadow fixer gadget onto the stack (add rsp, 20h; pop rdi; ret)
    push rbx										   ; Push NtWaitForSingleObject address onto the stack
    mov r10, [rsp + 170h]                              ; Set up the first argument as the third timer object (arg10 - HANDLE) (40x3 + 8x21 + 80 (50h))
    push r10                                           ; Push timer handle onto the stack
    push rcx                                           ; Push rcx gadget onto the stack (pop rcx; ret)
    ...
    push r12                                           ; Push shadow fixer gadget onto the stack (add rsp, 20h; pop rdi; ret)
    push rbx										   ; Push NtWaitForSingleObject address onto the stack
    mov r10, [rsp + 1D8h]                              ; Set up the first argument as the second timer object (arg9 - HANDLE) (40x4 + 8x30 + 72 (48h))
    push r10                                           ; Push timer handle onto the stack
    push rcx                                           ; Push rcx gadget onto the stack (pop rcx; ret)
    ...
    push r12                                           ; Push shadow fixer gadget onto the stack (add rsp, 20h; pop rdi; ret)
    mov rcx, [rsp + 238h]                              ; Set up the first argument as the first timer object (arg8 - HANDLE) (40x5 + 8x38 + 64 (40h))
